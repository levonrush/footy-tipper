...
def comp_placement_metrics(probabilities, market, y, groups, rows=None):
    ...
    Reporting only. This does not select the model, and the commit history
    explains at length why: mean per-season P(first) is close to bimodal, so a
    handful of seasons swings it, and three walk-forward runs that selected on
    it produced models worse than the deployed one. It is the right number to
    look at and the wrong number to optimise.

    Tips are `probabilities > 0.5`; the rival field tips the market favourite
    with per-rival flip noise, as `comp_sim` models it. Within one realized
    season P(first) is monotone in tips correct, because rival scores do not
    depend on our tips. Across seasons it is not the same as pooled accuracy,
    since P(first) saturates, so seasons are scored separately and averaged.
    ...
    return {
        ...
        "mean_expected_rank": float(
            np.mean([season["expected_rank"] for season in seasons])
        ),
