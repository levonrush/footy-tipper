"""Static site generation: current round tips, archive, and season results.

Writes plain HTML (no build step) into docs/site/ so the repo can serve it
via GitHub Pages (Settings -> Pages -> main branch, /docs folder).
"""

import html
import subprocess
from datetime import datetime, timezone
from pathlib import Path

import pandas as pd

from pipeline.common.use_predictions.distribution import get_predictions
from pipeline.common.use_predictions.email_render import (
    _distribution_for,
    _finals_field,
    _finals_on,
    _format_predicted_margin,
    _format_predicted_score_numbers,
    _format_market_price,
    _format_price,
    _format_probability,
    _head_to_head_for,
    _joker_reader_lines,
    _margin_band_text,
    _market_coverage_notice,
    _prediction_winner,
    _stakes_for,
    _why_text,
    tip_probability,
)
from pipeline.common.rounds import round_slug
from pipeline.common.use_predictions.finals import finals_payload
from pipeline.common.use_predictions.joker import get_joker_round_recommendation
from pipeline.common.use_predictions.scoreboard import (
    get_season_results,
    get_season_scoreboard,
)
from pipeline.common.use_predictions.staking import get_tipper_picks

_PAGE_CSS = """
:root { color-scheme: light; }
* { box-sizing: border-box; }
body { margin: 0; background: #eef2f7; color: #0f172a;
       font-family: 'Trebuchet MS', Arial, sans-serif; }
.wrap { max-width: 820px; margin: 0 auto; padding: 24px 16px 48px; }
header.site { background: linear-gradient(135deg, #115e59 0%, #0369a1 100%);
              border-radius: 12px; padding: 22px 24px; color: #fff; }
header.site h1 { margin: 0; font-size: 26px; letter-spacing: 0.5px; }
header.site p { margin: 6px 0 0; color: #cffafe; font-size: 14px; }
nav { margin: 14px 0 22px; }
nav a { color: #0f766e; font-weight: 700; text-decoration: none; margin-right: 16px; font-size: 14px; }
nav a:hover { text-decoration: underline; }
.card { background: #fff; border-radius: 12px; padding: 18px 20px; margin: 0 0 18px;
        box-shadow: 0 1px 2px rgba(15, 23, 42, 0.06); }
.card h2 { margin: 0 0 12px; font-size: 19px; border-left: 4px solid #0f766e; padding-left: 10px; }
table { width: 100%; border-collapse: collapse; font-size: 14px; }
th { text-align: left; padding: 8px 10px; color: #64748b; font-size: 12px;
     text-transform: uppercase; letter-spacing: 0.4px; border-bottom: 2px solid #e2e8f0; }
td { padding: 9px 10px; border-bottom: 1px solid #f1f5f9; }
tr:nth-child(even) td { background: #f8fafc; }
.tip { color: #0f766e; font-weight: 700; }
.why { display: block; margin-top: 3px; font-weight: 400; font-size: 12px; color: #64748b; }
.why-block { margin: 0 0 10px; color: #334155; font-size: 14px; }
.card h3 { margin: 16px 0 6px; font-size: 14px; color: #475569; text-transform: uppercase;
           letter-spacing: 0.4px; }
.num { text-align: right; font-variant-numeric: tabular-nums; white-space: nowrap; }
.barcell { width: 45%; }
.bar { display: inline-block; height: 10px; border-radius: 3px; min-width: 2px; }
.bar.pos { background: #0f766e; }
.bar.neg { background: #b91c1c; }
.flags { margin: 0 0 8px; font-size: 12px; color: #9a3412; }
.chain { margin: 8px 0 0; font-size: 12px; color: #64748b; font-variant-numeric: tabular-nums; }
.badge { display: inline-block; padding: 2px 8px; border-radius: 12px; font-size: 12px; font-weight: 700; }
.badge.hi  { background: #dcfce7; color: #15803d; }
.badge.med { background: #fef9c3; color: #854d0e; }
.badge.lo  { background: #fee2e2; color: #b91c1c; }
.hit  { color: #15803d; font-weight: 700; }
.miss { color: #b91c1c; font-weight: 700; }
.stats { display: flex; gap: 12px; flex-wrap: wrap; }
.stat { flex: 1 1 140px; background: #f0fdfa; border: 1px solid #99f6e4; border-radius: 10px;
        padding: 12px; text-align: center; }
.stat .v { font-size: 22px; font-weight: 700; }
.stat .l { margin-top: 2px; color: #64748b; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; }
.joker { border-radius: 10px; border: 1px solid #f59e0b; background: #fff7ed; padding: 14px 16px; }
.joker.play { border-color: #10b981; background: #ecfdf5; }
.joker h3 { margin: 0 0 8px; font-size: 16px; }
.joker p { margin: 0 0 6px; font-size: 14px; color: #1f2937; }
footer { margin-top: 26px; color: #94a3b8; font-size: 12px; text-align: center; }
.ribbon { margin: 0 0 10px !important; font-size: 12px; font-weight: 700;
          letter-spacing: 2px; text-transform: uppercase; }
.stakes { display: block; margin-top: 4px; font-size: 12px; font-weight: 700;
          letter-spacing: 0.4px; text-transform: uppercase; }
.context { display: block; margin-top: 3px; font-weight: 400; font-size: 12px; color: #64748b; }
"""


def _badge_class(tip_prob):
    if pd.isna(tip_prob):
        return "med"
    if tip_prob >= 0.70:
        return "hi"
    if tip_prob >= 0.55:
        return "med"
    return "lo"


def _page(title, subtitle, body, generated_note=True, theme=None):
    stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    footer = f"Generated by Footy Tipper on {stamp}. Tips are for bragging rights, not financial advice." if generated_note else ""
    header_style = ""
    ribbon = ""
    if isinstance(theme, dict) and theme.get("ribbon_label"):
        header_style = f' style="background:{theme["header_gradient"]}"'
        ribbon = (
            f'<p class="ribbon" style="color:{theme["ribbon_text"]}">'
            f"{html.escape(str(theme['ribbon_label']))}</p>"
        )
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{html.escape(title)}</title>
<style>{_PAGE_CSS}</style>
</head>
<body>
<div class="wrap">
<header class="site"{header_style}>
{ribbon}<h1>Footy Tipper</h1>
<p>{html.escape(subtitle)}</p>
</header>
<nav><a href="index.html">This Round</a><a href="results.html">Season Results</a><a href="explain.html">Why</a></nav>
{body}
<footer>{footer}</footer>
</div>
</body>
</html>
"""


def _scoreboard_card(scoreboard):
    if not isinstance(scoreboard, dict):
        return ""
    stats = [
        (f"{scoreboard['last_round_correct']}/{scoreboard['last_round_games']}", scoreboard["last_round_name"]),
        (
            f"{scoreboard['season_correct']}/{scoreboard['season_games']} ({scoreboard['season_accuracy']:.0%})",
            "Season",
        ),
    ]
    if scoreboard.get("market_accuracy") is not None:
        stats.append((f"{scoreboard['market_accuracy']:.0%}", "Market favourite"))
    cells = "".join(
        f'<div class="stat"><div class="v">{html.escape(v)}</div><div class="l">{html.escape(label)}</div></div>'
        for v, label in stats
    )
    return f'<div class="card"><h2>The ledger</h2><div class="stats">{cells}</div></div>'


def _fixture_context_html(finals, row):
    """Knockout stakes, margin shape and head-to-head, under a finals fixture."""
    if not _finals_on(finals):
        return ""
    stakes = _stakes_for(finals, row)
    blocks = []
    if stakes:
        accent = finals["theme"]["accent"]
        blocks.append(
            f'<span class="stakes" style="color:{accent}">{html.escape(str(stakes))}</span>'
        )
    for text in (
        _margin_band_text(_distribution_for(finals, row)),
        _head_to_head_for(finals, row),
    ):
        if text:
            blocks.append(f'<span class="context">{html.escape(str(text))}</span>')
    return "".join(blocks)


def _premiership_card(race, theme):
    """The site twin of the email's premiership table."""
    if not isinstance(race, dict):
        return ""
    if not race.get("available"):
        return (
            '<div class="card"><h2>Road to the big dance</h2>'
            "<p>Premiership probabilities are unavailable this week.</p></div>"
        )
    rows = "".join(
        "<tr>"
        f"<td>{team['seed']}</td>"
        f"<td>{html.escape(str(team['team']))}</td>"
        f"<td class=\"num\">{_format_probability(team['p_grand_final'])}</td>"
        f"<td class=\"num\">{_format_probability(team['p_premiership'])}</td>"
        "</tr>"
        for team in race["teams"]
        if team["alive"]
    )
    accent = (theme or {}).get("feature_accent", "#0f766e")
    return (
        f'<div class="card"><h2 style="border-left-color:{accent}">Road to the big dance</h2>'
        "<table><thead><tr><th>Seed</th><th>Team</th><th>Grand Final</th>"
        "<th>Premiership</th></tr></thead>"
        f"<tbody>{rows}</tbody></table>"
        f'<p class="context">{html.escape(str(race.get("method", "")))} '
        f"Based on {race['simulations']:,} simulated finals series.</p></div>"
    )


def _market_card(market_picks):
    """Line and totals picks. Finals only."""
    if market_picks is None or getattr(market_picks, "empty", True):
        return ""
    rows = "".join(
        "<tr>"
        f"<td>{html.escape(str(pick['market']))}</td>"
        f"<td>{html.escape(str(pick['selection']))}</td>"
        f"<td>{_format_price(pick['price'])}</td>"
        f"<td>{_format_price(pick['price_min'])}</td>"
        f"<td class=\"num\">{float(pick['edge']):.1%}</td>"
        f"<td class=\"num\">{float(pick['stake_fraction']):.1%}</td>"
        "</tr>"
        for _, pick in market_picks.iterrows()
    )
    return (
        '<div class="card"><h2>Line and totals</h2>'
        "<table><thead><tr><th>Market</th><th>Selection</th><th>Price</th><th>Fair</th>"
        "<th>Edge</th><th>Stake share</th></tr></thead>"
        f"<tbody>{rows}</tbody></table></div>"
    )


def _tips_card(predictions, finals=None):
    if predictions.empty:
        return (
            '<div class="card"><h2>This round</h2>'
            "<p>No upcoming pre-game fixtures right now. Check back when the next round's team lists drop.</p></div>"
        )

    rows = []
    for _, row in predictions.iterrows():
        winner = _prediction_winner(row)
        tip_prob = tip_probability(row)
        why = _why_text(row)
        why_html = f'<span class="why">{html.escape(why)}</span>' if why else ""
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(row['team_home']))} vs {html.escape(str(row['team_away']))}"
            f"{_fixture_context_html(finals, row)}</td>"
            f"<td class=\"tip\">{html.escape(str(winner))}{why_html}</td>"
            f"<td><span class=\"badge {_badge_class(tip_prob)}\">{_format_probability(tip_prob)}</span></td>"
            f"<td>{html.escape(_format_predicted_score_numbers(row))}</td>"
            f"<td>{html.escape(_format_predicted_margin(row))}</td>"
            f"<td>H {_format_market_price(row['team_head_to_head_odds_home'], row.get('market_odds_fresh', True))}"
            f" / A {_format_market_price(row['team_head_to_head_odds_away'], row.get('market_odds_fresh', True))}</td>"
            "</tr>"
        )
    round_name = _finals_field(finals, "display_name") or predictions["round_name"].iloc[0]
    year = predictions["competition_year"].iloc[0]
    market_notice = _market_coverage_notice(predictions)
    market_notice_html = ""
    if market_notice:
        market_notice_html = (
            '<p style="padding:10px 12px; border:1px solid #fdba74; '
            'border-radius:8px; background:#fff7ed; color:#9a3412;">'
            f"<strong>Market data notice:</strong> {html.escape(market_notice)}</p>"
        )
    return (
        f'<div class="card"><h2>{html.escape(str(round_name))} {html.escape(str(year))} tips</h2>'
        f"{market_notice_html}"
        "<table><thead><tr><th>Fixture</th><th>Tip</th><th>Confidence</th><th>Score tip</th>"
        "<th>Margin</th><th>H2H odds</th></tr></thead>"
        f"<tbody>{''.join(rows)}</tbody></table></div>"
    )


def _value_card(tipper_picks):
    if tipper_picks.empty:
        return '<div class="card"><h2>Value picks</h2><p>None flagged this round.</p></div>'
    rows = []
    for _, row in tipper_picks.iterrows():
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(row['team']))} vs {html.escape(str(row['opponent']))}</td>"
            f"<td>{_format_price(row['price'])}</td>"
            f"<td>{_format_price(row['price_min'])}</td>"
            f"<td>{float(row['edge']):.1%}</td>"
            f"<td>{float(row['stake_fraction']):.1%}</td>"
            "</tr>"
        )
    return (
        '<div class="card"><h2>Value picks</h2>'
        "<table><thead><tr><th>Team</th><th>Market</th><th>Fair</th><th>Edge</th><th>Stake share</th></tr></thead>"
        f"<tbody>{''.join(rows)}</tbody></table></div>"
    )


def _context_watch_card(context_cards):
    """Render immutable, sourced Club Context facts in shadow mode."""
    try:
        from pipeline.common.club_context.product import (
            SHADOW_DISCLAIMER,
            category_label,
            normalize_context_cards,
        )

        cards = normalize_context_cards(context_cards)
    except Exception:
        return ""
    if not cards:
        return ""

    items = []
    for card in cards:
        owner = f"{card['team_name']} · " if card.get("team_name") else ""
        phase = str(card.get("phase") or "").replace("_", " ").strip()
        title = owner + category_label(card.get("category"))
        if phase:
            title += f" · {phase.title()}"
        items.append(
            '<div style="border-top:1px solid #bfdbfe; padding-top:10px; margin-top:10px;">'
            f'<h3 style="margin:0 0 5px; color:#0f172a;">{html.escape(title)}</h3>'
            f'<p style="margin:0 0 5px; line-height:1.5;">{html.escape(card["display_copy"])}</p>'
            f'<p style="margin:0; color:#64748b; font-size:12px;">Source: '
            f'<a style="color:#0369a1;" href="{html.escape(card["source_url"], quote=True)}" '
            f'rel="noopener noreferrer">{html.escape(card["source_title"])}</a></p>'
            "</div>"
        )
    return (
        '<div class="card" style="border:1px solid #93c5fd; background:#eff6ff;">'
        '<h2 style="border-left-color:#0369a1;">Context Watch</h2>'
        f'<p style="color:#64748b; font-size:12px;">{html.escape(SHADOW_DISCLAIMER)}</p>'
        + "".join(items)
        + "</div>"
    )


def _joker_card(joker_recommendation):
    if not isinstance(joker_recommendation, dict):
        return ""
    headline = html.escape(str(joker_recommendation.get("headline", "Joker call unavailable")))
    body = "".join(f"<p>{html.escape(line)}</p>" for line in _joker_reader_lines(joker_recommendation))
    play_class = " play" if joker_recommendation.get("should_use_this_round") else ""
    return f'<div class="card"><h2>Joker round call</h2><div class="joker{play_class}"><h3>{headline}</h3>{body}</div></div>'


def _results_card(results):
    if results.empty:
        return '<div class="card"><h2>Season results</h2><p>No settled predicted games yet this season.</p></div>'

    rows = []
    for _, row in results.iterrows():
        if bool(row.get("is_draw", False)):
            outcome = '<span class="badge med">Draw</span>'
        elif bool(row["tip_correct"]):
            outcome = '<span class="hit">&#10003;</span>'
        else:
            outcome = '<span class="miss">&#10007;</span>'
        score = f"{int(row['team_final_score_home'])}&ndash;{int(row['team_final_score_away'])}"
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(row['round_name'] or f'Round {int(row.round_id)}'))}</td>"
            f"<td>{html.escape(str(row['team_home']))} vs {html.escape(str(row['team_away']))}</td>"
            f"<td class=\"tip\">{html.escape(str(row['tipped_team']))} ({_format_probability(row['tip_prob'])})</td>"
            f"<td>{score}</td>"
            f"<td>{outcome}</td>"
            "</tr>"
        )
    return (
        '<div class="card"><h2>Season results</h2>'
        "<table><thead><tr><th>Round</th><th>Fixture</th><th>Tip</th><th>Score</th><th>Hit</th></tr></thead>"
        f"<tbody>{''.join(rows)}</tbody></table></div>"
    )


def _bar(points, scale):
    """A CSS-only signed bar. No JS: these pages are served as static files."""
    width = 0 if scale <= 0 else min(100, abs(points) / scale * 100.0)
    css_class = "pos" if points >= 0 else "neg"
    return f'<span class="bar {css_class}" style="width:{width:.0f}%"></span>'


def _driver_rows(drivers, unit):
    if not drivers:
        return '<tr><td colspan="3">Nothing above the noise floor.</td></tr>'
    scale = max(abs(d.points) for d in drivers) or 1.0
    return "".join(
        "<tr>"
        f"<td>{html.escape(d.label)}</td>"
        f"<td class=\"num\">{d.points:+.2f} {html.escape(unit)}</td>"
        f'<td class="barcell">{_bar(d.points, scale)}</td>'
        "</tr>"
        for d in drivers
    )


def _explain_game_card(explanation):
    probability = explanation.probability
    score = explanation.score
    chain = (
        f"pooled logit {probability.pooled_logit:+.4f}"
        f" &divide; T {probability.temperature:.4f}"
        f" = {probability.calibrated_logit:+.4f}"
        f" &rarr; {probability.published_cond:.1%}"
    )
    experts = "".join(
        "<tr>"
        f"<td>{html.escape(name)}</td>"
        f"<td class=\"num\">{getattr(probability, name, float('nan')):.4f}</td>"
        f"<td class=\"num\">{weight:.3f}</td>"
        f"<td class=\"num\">{probability.expert_logit_terms.get(name, 0.0):+.4f}</td>"
        "</tr>"
        for name, weight in sorted(probability.weights.items())
    )
    flags = []
    if probability.guard_fired:
        flags.append("consensus guard fired")
    if score.reconciled:
        flags.append("scoreline reconciled")
    if score.line_applied:
        flags.append("market line applied")
    if score.total_applied:
        flags.append("market total applied")
    flag_html = (
        f'<p class="flags">{html.escape(", ".join(flags))}</p>' if flags else ""
    )

    return (
        '<div class="card">'
        f"<h2>{html.escape(explanation.team_home)} vs {html.escape(explanation.team_away)}</h2>"
        f"<p><strong>{html.escape(explanation.tipped_team)}</strong> at "
        f"{probability.published_cond:.0%} &middot; {score.displayed_home}-{score.displayed_away}</p>"
        f"<p class=\"why-block\">{html.escape(explanation.why_line)}</p>"
        f"{flag_html}"
        "<h3>Probability drivers</h3>"
        f"<table><tbody>{_driver_rows(explanation.prob_families, 'pts')}</tbody></table>"
        "<h3>Margin drivers</h3>"
        f"<table><tbody>{_driver_rows(explanation.margin_families, 'pt')}</tbody></table>"
        "<h3>How the number was made</h3>"
        "<table><thead><tr><th>Expert</th><th>Probability</th><th>Weight</th>"
        "<th>Logit term</th></tr></thead>"
        f"<tbody>{experts}</tbody></table>"
        f'<p class="chain">{chain} &middot; decided by '
        f"{html.escape(probability.attribution_source)} via the "
        f"{html.escape(probability.route)} route</p>"
        "</div>"
    )


def _explain_body(db_path, project_root):
    """The Why page. Degrades to a single explanatory card when empty."""
    try:
        from pipeline.common.explain import store as explain_store

        explanations = explain_store.load_game_explanations(db_path)
    except Exception as exc:
        explanations = []
        print(f"Explain page data unavailable ({exc}).")

    if not explanations:
        return (
            '<div class="card"><h2>Why</h2>'
            "<p>No stored explanations yet. They are written alongside the tips "
            "whenever inference runs.</p></div>"
        )

    intro = (
        '<div class="card"><h2>Why the model thinks what it thinks</h2>'
        "<p>Each tip is broken down into the feature families that moved it, in "
        "percentage points of win probability and points of margin, signed "
        "toward the tipped team. The expert table underneath shows how the "
        "published number was actually assembled.</p></div>"
    )
    return intro + "".join(_explain_game_card(e) for e in explanations)


def generate_site(db_path, project_root):
    """Render docs/site/ pages. Returns the list of files written."""
    project_root = Path(project_root)
    site_dir = project_root / "docs" / "site"
    rounds_dir = site_dir / "rounds"
    site_dir.mkdir(parents=True, exist_ok=True)
    rounds_dir.mkdir(parents=True, exist_ok=True)

    predictions = get_predictions(db_path, project_root)
    tipper_picks = get_tipper_picks(predictions) if not predictions.empty else pd.DataFrame()
    joker = get_joker_round_recommendation(db_path, project_root, predictions) if not predictions.empty else None
    scoreboard = get_season_scoreboard(db_path)
    results = get_season_results(db_path)
    finals = finals_payload(db_path, predictions) if not predictions.empty else None
    context_cards = []
    if not predictions.empty and "game_id" in predictions:
        try:
            from pipeline.common.club_context.product import load_context_cards

            context_cards = load_context_cards(db_path, predictions["game_id"].tolist())
        except Exception as exc:
            print(f"Club Context site data unavailable ({exc}).")
    is_finals = _finals_on(finals)
    theme = finals["theme"] if is_finals else None

    if not predictions.empty:
        round_name = _finals_field(finals, "display_name") or str(
            predictions["round_name"].iloc[0]
        )
        year = predictions["competition_year"].iloc[0]
        subtitle = f"{round_name} {year} — machine-made NRL tips"
    else:
        subtitle = "Machine-made NRL tips"

    # The joker slot becomes the premiership race once the comp has finished.
    feature_card = (
        _premiership_card(finals.get("premiership"), theme)
        if is_finals
        else _joker_card(joker)
    )
    index_body = (
        _scoreboard_card(scoreboard)
        + _tips_card(predictions, finals)
        + _context_watch_card(context_cards)
        + _value_card(tipper_picks)
        + (_market_card(finals.get("market_picks")) if is_finals else "")
        + feature_card
    )
    index_html = _page("Footy Tipper — This Round", subtitle, index_body, theme=theme)

    written = []
    index_path = site_dir / "index.html"
    index_path.write_text(index_html, encoding="utf-8")
    written.append(index_path)

    if not predictions.empty:
        try:
            year_val = int(predictions["competition_year"].iloc[0])
            slug = round_slug(
                predictions["round_name"].iloc[0], predictions["round_id"].iloc[0]
            )
            round_path = rounds_dir / f"{year_val}-{slug}.html"
            round_path.write_text(index_html, encoding="utf-8")
            written.append(round_path)
        except Exception as exc:
            print(f"Round archive page skipped ({exc}).")

    results_body = _scoreboard_card(scoreboard) + _results_card(results)
    results_path = site_dir / "results.html"
    results_path.write_text(_page("Footy Tipper — Season Results", subtitle, results_body), encoding="utf-8")
    written.append(results_path)

    explain_path = site_dir / "explain.html"
    explain_path.write_text(
        _page("Footy Tipper — Why", subtitle, _explain_body(db_path, project_root)),
        encoding="utf-8",
    )
    written.append(explain_path)

    for path in written:
        print(f"Site page written: {path.relative_to(project_root)}")
    return written


def publish_site(project_root):
    """Commit and push docs/site/. No-op when there is nothing to commit."""
    project_root = Path(project_root)
    try:
        status = subprocess.run(
            ["git", "status", "--porcelain", "docs/site"],
            cwd=project_root, capture_output=True, text=True, check=True,
        ).stdout.strip()
        if not status:
            print("Site publish: nothing to commit.")
            return True
        subprocess.run(["git", "add", "docs/site"], cwd=project_root, check=True)
        subprocess.run(
            ["git", "commit", "-m", "Publish tips site update"],
            cwd=project_root, check=True,
        )
        subprocess.run(["git", "push"], cwd=project_root, check=True)
        print("Site publish: committed and pushed docs/site.")
        return True
    except subprocess.CalledProcessError as exc:
        print(f"Site publish failed ({exc}).")
        return False
