...
### Added
- Finals special editions (see [docs/finals-edition.md](docs/finals-edition.md)). The tipping comps stop at the end of the regular season, so the four finals weeks now run as a distinct email rather than a weekly one that has lost its point. The joker and the competition-strategy layer are suppressed, because both optimise a comp that has finished, and the slot they occupied becomes **Road to the big dance**: a seeded 20,000-run Monte Carlo over the remaining bracket giving every surviving team a premiership and grand-final probability. Drawn fixtures are priced by the calibrated model; matchups that do not exist yet by Tier-A ratings, and the email says which is which. Each stage gets its own subject line, accent palette, SPECIAL EDITION ribbon and banner brief, and each fixture gains its knockout stakes, its margin shape and the season series between the two clubs. Value picks extend to the line and totals markets.
- `pipeline/common/rounds.py`, one round-stage classifier shared by ingestion, prediction and delivery. It recognises both the draw vocabulary (`Finals Week 1`) and the older broadcast vocabulary lineup articles use (`Qualifying Final`), treats an unrecognised finals-ish name as finals rather than regular, and trusts round numbers only when the caller supplies the season's last regular round, because finals round numbers move between seasons.
- `prediction_distributions`, a sibling diagnostics table on the same terms as `prediction_explanations`: written during inference from the samples the tip already came from, so it never re-simulates and cannot move a prediction, and a failure costs a section rather than a send. It carries margin bands, one-score and blowout probabilities, total quantiles and cover probabilities against the posted line and total, and it is what the margin shape and the new line/totals picks read.
- `compute_tier_a_ratings`, exposing the end state of the Tier-A walk so an undrawn matchup can be priced, with a Platt scaling fitted from that same leak-safe walk. Raw Tier-A probabilities are badly overconfident: the 0.9 to 1.0 bucket wins about 72% of the time and nearly half of all games land outside `[0.1, 0.9]`, so simulating a bracket on them would have reported a minor premier as an 85% chance. Calibrated, the Brier score moves from 0.274 to 0.225 on 2015 onward with the ordering unchanged. At a neutral venue only the slope applies, which is what makes the two sides of a grand final sum to one.
- `FOOTY_TIPPER_FINALS_MODE` (`auto`, `on`, `off`), forcing or disabling the finals treatment so it can be rehearsed out of season.
...
### Fixed
- The finals email copy was dead code. `_special_event_context` matched `"qualifying final"`, `"elimination final"`, `"semi final"` and `"preliminary final"`, none of which the nrl.com draw has ever emitted: it says `Finals Week 1/2/3`. Every finals week except the decider fell through to the regular-round opener, *"Another week, another chance to make objectively questionable tipping decisions"*.
- Finals fixtures could be listed out of order. The published view did not select `start_time` or `game_number`, so display fell back to `game_id`, which is kickoff order in a regular round but bracket order in the finals.
- The ladder could keep accumulating past the finals cutover. `is_regular_round` trusts `round_name`, and the draw synthesises `Round N` when `roundTitle` is missing, so the `round_id <= 27` fallback could never fire; a single unnamed finals round would have kept scoring games and awarded bye points to every eliminated team. A season now enters the finals once and cannot come back out.
...
- The lineup as-of cutoff compared a venue-local kickoff against true-UTC article publish times. `start_time` is venue-local wall clock serialised as-if-UTC while `source_published_at_utc` is true UTC, so the documented 24 hour leakage guard was really running at about 13 to 14 hours for Australian venues. The cutoff is now measured from `start_time_utc`. Verified inference-neutral: on the live round the 135 lineup feature columns are identical under both, because the pre-game cutoff is `min(now, kickoff)` and kickoff is in the future either way. `lineup_source_age_hours` is computed on a separate path and still carries the same inflation; it is a deployed predictor, so correcting it changes a feature value under the shipped pickles and is deferred to land with a retrain.
