...
home_form <- home_games %>%
  group_by(team, competition_year) %>%
  mutate(home_form = ifelse(seq(n()) < 5,
                            cumsum(case_when(
                              home_team_result == 'Win'  ~ 1, 
                              home_team_result == 'Loss' ~ -1,
                              TRUE                       ~ 0)),
                            rollsum(x = case_when(
                              ...
                              home_team_result == 'Loss' ~ -1,
                              TRUE                       ~ 0), 5, align = "right", fill = 0)),
           form_points_for = ifelse(seq(n()) < 5,
                                    cumsum(points_for),
                                    rollapply(points_for, 5, mean, align = 'right', fill = 0)),
           form_points_against = ifelse(seq(n()) < 5,
                                        cumsum(points_for),
                                        rollapply(points_for, 5, mean, align = 'right', fill = 0)),
           form_diff = ifelse(seq(n()) < 5,
                              cumsum(points_for - points_against),
                              rollapply(points_for, 5, mean, align = 'right', fill = 0))) %>%
  mutate_at(vars(home_form, form_points_for, form_points_against, form_diff), lag) %>%
  replace(is.na(.), 0) %>%
  ungroup() %>%
  select(-c(competition_year, home_team_result))

away_form <- footy_tipper_df %>%
  select(game_id, competition_year, team_away, team_final_score_home, team_final_score_away, home_team_result) %>%
  rename(team = team_away, points_for = team_final_score_away, points_against = team_final_score_home) %>%
  arrange(game_id) %>%
  group_by(team, competition_year) %>%
  mutate(away_form = ifelse(seq(n()) < 5,
                            cumsum(case_when(
                              home_team_result == 'Win'  ~ -1, 
                              home_team_result == 'Loss' ~ 1,
                              TRUE                       ~ 0)),
           ...
           form_diff = ifelse(seq(n()) < 5,
                              cumsum(points_for - points_against),
                              rollapply(points_for, 5, mean, align = 'right', fill = 0))) %>%
  mutate_at(vars(away_form, form_points_for, form_points_against, form_diff), lag) %>%
  replace(is.na(.), 0) %>%
  ungroup() %>%
  select(-c(competition_year, home_team_result))

season_form <- home_form %>%
  left_join(away_form, by = "game_id", suffix = c("_home", "_away")) %>%
  mutate(season_form = ifelse(seq(n()) < 5,
                              home_form + away_form,
                              rollsum(home_form + away_form, 5, align = "right", fill = 0)))
         
         
         
         
         ,
         form_points_for_season = ifelse(seq(n()) < 5,
                                    cumsum(form_points_for_home + form_points_for_away),
                                    rollapply(form_points_for_home + form_points_for_away, mean, 5, align = "right", fill = 0)),
         form_points_against_season = ifelse(seq(n()) < 5,
                                        cumsum(form_points_against_home + form_points_against_away),
                                        rollapply(form_points_against_home + form_points_against_away, mean, 5, align = "right", fill = 0)),
         form_points_diff_season = ifelse(seq(n()) < 5,
                                         form_points_for_season - form_points_against_season,
                                         rollapply(points_against_home + points_against_away, mean, 5, align = "right", fill = 0)))
         
         

         form_diff_season = ifelse(seq(n()) < 5,
                                   points_against_home + points_against_away,
                                   rollsum(points_against_home + points_against_away, 5, align = "right", fill = 0)))
```
